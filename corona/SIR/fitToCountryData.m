function outsideFilename = fitToCountryData(MATdata)
    ...
    disp('-------------------')
    disp('\hline')
	disp('Country & $w$ & $\sigma_w$ & p(t-test $H_0:w=1$) & $w_{{\rm 2-delay}}$ & p(same $H_0$) \\')
    disp('\hline')
    disp('\hline')
    for ctry = 1:length(countryStrings)
        ...
        disp([countryStr,' & ',...
            num2str(exp(-abs(fit.fullsolution(6)))),' & ',...
            num2str(StErr),' & ',...
            num2str(fit.fullpValues(6)),' & ',...
            num2str(exp(-abs(fit.ddsolution(7)))),' & ',...
            num2str(fit.ddpValues(7)),...
            ' \\']);
