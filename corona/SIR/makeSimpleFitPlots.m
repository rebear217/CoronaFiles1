...
load('../data/USdeathData.mat')
clc
close all

%%

makeDeathsPerHeadPlot(MATdata,'United Kingdom');

%%

close all
deathsPerInfection(MATdata);
...
close all
ignoreUS = {true,false};
[~,loglogestimateNoUS] = loglogPlot(MATdata,ignoreUS{1});
[~,loglogestimate] = loglogPlot(MATdata,ignoreUS{2});
...
close all
[UKdeaths,UKAIC,UKRLmatrix] = OneCountryIsolationAnalysis(MATdata,'United Kingdom',IG_UK);
%close all
...
%close all
%CAdeaths = OneCountryIsolationAnalysis(USdata,'California',IG_CA,56,[1 2 1 2 1]);
%close all
...
close all
[Swedendeaths,SwedenAIC,SwedenRLmatrix] = OneCountryIsolationAnalysis(MATdata,'Sweden',IG_Sweden);
...
UKestimateVector = [loglogestimateNoUS,...
    loglogestimate,...
    DSDestimate(1),...
    DSDestimate(2),...
    DSDestimate99(1),...
